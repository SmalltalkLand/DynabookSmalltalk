...
sqcogspur64linuxht/bin/squeak -vm-display-null Cuis5.0-4426.image -d "Smalltalk at: #Auto put: #x.BaseLine fileOutSystem."  -d "Smalltalk at: #Auto put: #x.Smalltalk snapshot: true andQuit: true clearAllClassState: true then: [Smalltalk quitPrimitive: 1]."
#sqcogspur64linuxht/bin/squeak -vm-display-null Cuis5.0-4426.image -d "Smalltalk at: #Auto put: #x.[(Delay forSeconds: 290) wait. StdIOWriteStream stdout newLine; nextPutAll: 'Failed build, continuing'.Smalltalk quitPrimitive: 1] fork. \"BaseLine fileOutSystem.\" [Smalltalk reset: [Smalltalk snapshot: true andQuit: true. ]] on: Error do: [:ex|StdIOWriteStream stdout newLine; nextPutAll: ex printString]."
